...
# 先删除远程目录以确保完全同步
echo "🗑️ 正在清理远程目录..."
sshpass -p "$REMOTE_PASS" ssh $SSH_OPTS "$REMOTE_USER@$REMOTE_HOST" "rm -rf $REMOTE_PROJECT_DIR"
...
# 解压远程文件
echo "🔧 正在解压远程文件..."
sshpass -p "$REMOTE_PASS" ssh $SSH_OPTS "$REMOTE_USER@$REMOTE_HOST" "cd /tmp && tar xzf $TEMP_TAR && rm -f $TEMP_TAR && mv $LOCAL_DIR $REMOTE_DIR/"
...
# 在远程主机安装Python依赖
# ==============================
echo "🐍 正在远程主机上安装Python依赖..."
sshpass -p "$REMOTE_PASS" ssh $SSH_OPTS "$REMOTE_USER@$REMOTE_HOST" "cd $REMOTE_PROJECT_DIR && pip install -r requirements.txt"

if [ $? -eq 0 ]; then
    echo "✅ 目录复制完成，Python依赖安装成功"
else
    echo "❌ Python依赖安装失败"
    exit 1
fi